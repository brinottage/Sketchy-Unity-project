...
public class Ally : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		if (life <= 0)
		{
			...
			if (isBoss){
				GameObject.Find("PasarJuego").GetComponent<BoxCollider2D>().enabled = true;
				GameObject.Find("PasarJuego").GetComponent<SpriteRenderer>().enabled = true;
				GameObject.Find("flecha").GetComponent<SpriteRenderer>().enabled = true;
			}
		}
	...
	public void MeleeAttack()
	{
		...
		for (int i = 0; i < collidersEnemies.Length; i++)
		{
			...
			{
				collidersEnemies[i].gameObject.GetComponent<CharacterController2D>().ApplyDamage(1f, transform.position);
			}
		}
